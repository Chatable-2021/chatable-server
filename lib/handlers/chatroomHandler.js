...
const chatroomHandler = (io, socket) => {
  socket.on('CREATE_CHATROOM', chatroom => {
    Chatroom.create(chatroom)
      .then(rooms => {
        io.emit('ROOMS_RESULTS', { rooms, success: true });
      })
      .catch(err => {
  ...
  socket.on('JOIN_ROOM', ({ id, user }) => {
    Chatroom.findById(id)
      .then(room => {
        ...
        socket.broadcast.to(room.id).emit('BROADCAST_JOIN', {
  ...
  socket.on('CHAT_MESSAGE', ({ message, userId, userName, roomId }) => {
    Message.insert(roomId, userId, userName, message)
      .then(message => {
